using CustomMiddleware.Descriptions;
...
namespace CustomMiddleware
{
    public class SOAPEndpointMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext, IServiceProvider serviceProvider)
        {
            ...
            PathString _endpointPath = httpContext.Request.Path;
            if (httpContext.Request.Path.Equals(_endpointPath, StringComparison.Ordinal))
            {
                Message responseMessage;

                // Read request message
                var requestMessage = _encoder.ReadMessage(httpContext.Request.Body, 0x10000, httpContext.Request.ContentType);
                ...
                responseMessage = Message.CreateMessage(_encoder.MessageVersion, operation.ReplyAction, bodyWriter);

                httpContext.Response.ContentType = httpContext.Request.ContentType; // _messageEncoder.ContentType;
                httpContext.Response.Headers["SOAPAction"] = responseMessage.Headers.Action;

                _encoder.WriteMessage(responseMessage, httpContext.Response.Body);
            }
            else
            {
